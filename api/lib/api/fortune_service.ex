defmodule Api.FortuneService do
  ...
  def run_pipeline() do
    ...
    with {:ok, calc_data} <- get_chart_data_with_retry(),
         ...
         {:ok, fortune_text} <- generate_fortune_with_retry(prompt),
         {:ok, _notion_response} <- upload_to_notion_with_retry(fortune_text) do
  ...
  def upload_to_notion_with_retry(fortune_text) do
    with_retry(fn ->
      tz_offset = Float.parse(System.get_env("TARGET_TZ_OFFSET") || "9.0") |> elem(0)
      local_now = DateTime.add(DateTime.utc_now(), trunc(tz_offset * 3600), :second)
      local_date = DateTime.to_date(local_now)

      title = "#{local_date.year}-#{local_date.month}-#{local_date.day} 일일 운세"
      NotionClient.append_fortune(title, fortune_text)
    end)
  end
